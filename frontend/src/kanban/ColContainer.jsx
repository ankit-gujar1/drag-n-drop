...
const ColContainer = ({ col, deleteCol, updateTitle, createTask, tasks, deleteTask, updateTask }) => {
    ...
    if (isDragging) {
        return (
            <div style={style} ref={setNodeRef} className='bg-black text-white w-80 rounded-lg h-[32rem] flex flex-col opacity-40 border-2 border-red-500'></div>
        )
    }

    return (
        // key is IMP for smooth animation
        <div style={style} ref={setNodeRef} key={col.id} className='bg-slate-800 text-white w-80 rounded-lg h-[32rem] flex flex-col'>
            <div {...attributes} {...listeners} className='flex justify-around items-center py-5 bg-black rounded-lg rounded-b-none' onClick={() => setEditMode(true)}>
                <p>0</p>
                {!editMode && <p>{col.title}</p>}
                {editMode &&
            ...
            <div className='flex flex-grow flex-col gap-2 my-2 overflow-y-auto'>
                <SortableContext items={taskIds}>
                    {tasks && tasks.map((i) => (
                        <Task task={i} key={i.id} deleteTask={deleteTask} updateTask={updateTask} />
                    ))}
                </SortableContext>
            </div>
            <div className='flex justify-center items-center'>
                <button onClick={() => createTask(col.id)} className='flex justify-center items-center py-3 bg-black px-10 rounded-lg mb-5 my-2'>
